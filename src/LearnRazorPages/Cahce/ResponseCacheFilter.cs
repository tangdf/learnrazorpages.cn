...
namespace LearnRazorPages.Cahce
{
    ...
    public class ResponseCacheFilter : IPageFilter
    {
        ...
        internal bool IsOverridden(PageHandlerExecutingContext context)
        {
            if (context == null)
            {
            ...
            return context.Filters.OfType<ResponseCacheFilter>().Last() != this;
        }
        ...
        public void OnPageHandlerExecuting(PageHandlerExecutingContext context)
        {
            ...
            if (!NoStore)
            {
                ...
                if (_cacheProfile.Duration == null && _cacheDuration == null)
                {
                    throw new InvalidOperationException(
                            string.Format("If the '{0}' property is not set to true, '{1}' property must be specified.",nameof(NoStore), nameof(Duration)));
                }
            }
            ...
            if (VaryByQueryKeys != null)
            {
                ...
                {
                    throw new InvalidOperationException(string.Format("'{0}' requires the response cache middleware.",nameof(VaryByQueryKeys)));
                }
            ...
            else
            {
                ...
                if (cacheControlValue != null) {
                    //headers[HeaderNames.CacheControl] = cacheControlValue;
                     Task OutputCacheHealer(object state)
                    {
                        HttpContext c = (HttpContext) state;
                        if (c.Response.StatusCode == 200)
                            c.Response.Headers[HeaderNames.CacheControl] = cacheControlValue;
                        return Task.CompletedTask;
                    }

                    context.HttpContext.Response.OnStarting(OutputCacheHealer, context.HttpContext);
                }
            }
        }
